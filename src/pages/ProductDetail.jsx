  ...
  if (!product) {
    return (
      <main className="container py-5">
        ...
        <Link to="/products" className="btn btn-dark rounded-pill mt-3">Volver al catálogo</Link>
      </main>
    );
  }

  const badgeText = product.isOffer ? "Oferta" : product.isNew ? "Nuevo" : null;
  const increaseQuantity = () => {setQuantity((current) => Math.min(current + 1, product.stock));};
  const decreaseQuantity = () => {setQuantity((current) => Math.max(current - 1, 1));};
  ...
  return (
    <main className="product-detail-page">
      <div className="container py-5">
        <Link to="/products" className="product-back-link">← Volver al catálogo</Link>
        <div className="row g-4 align-items-start mt-3">
          <div className="col-12 col-lg-6">
            <section className="product-detail-gallery">
              {badgeText && (
                <span className={`detail-badge ${product.isOffer ? "offer" : ""}`}>{badgeText}</span>
              )}
          ...
          <div className="col-12 col-lg-6">
            <section className="product-info-panel">
              ...
              <div className="product-detail-price-box">
                ...
                {product.oldPrice && (
                  <span className="product-detail-old-price">USD ${product.oldPrice}</span>
                )}
              </div>
